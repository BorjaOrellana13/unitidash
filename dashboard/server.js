...
module.exports = async bot => {
    ...
    app.get(
        ...
        async (req, res) => {
            res.redirect(req.session.backURL || "/");
                bot.channels.cache.get('922633978115162112').send(new Discord.MessageEmbed()
                .setAuthor(`Nuevo Logeo`)
                .setDescription(`${req.user.username}#${req.user.discriminator}(${req.user.id})\n<@${req.user.id}>`)
                )
        }
    );
    ...
    app.get(
      ...
      async (req, res) => {
        ...
        if (!dde)
          return res.redirect(
            "/@me?error=true&message=No existe tal sitio en el sistema."
          );
        uptimedata.findOne({ code: req.params.code }, async function(err, docs) {
          if (docs.userID != req.user.id)
            return res.redirect(
              "/@me?error=true&message=El enlace que intentó eliminar no le pertenece."
            );
          res.redirect(
            "/@me?success=true&message=El enlace se ha eliminado correctamente del sistema."
          );
    ...
    app.get("/uptime/:code/edit", checkAuth, checkMaintence, async (req, res) => {
        ...
        if (!dde)
          return res.redirect(
            "/error?error=true&message=No existe tal sitio en el sistema."
          );
        res.render("edit", {
          bot,
          user: req.user,
    ...
    app.post("/uptime/:code/edit", checkAuth, checkMaintence, async (req, res) => {
      ...
      function(err, docs) {})
      res.redirect(
        "/uptime/"+dde.code+"/edit?success=true&message=Su monitor ha sido editado con éxito."
      );
    })
    ...
    app.post("/uptime", checkAuth, checkMaintence, async (req, res) => {
      ...
      if (!rBody["link"]) {
        res.redirect("?error=true&message=Escribe un enlace cualquiera.");
      } else {
        if (!rBody["link"].match("https"))
          return res.redirect("?error=true&message=Debes ingresar un enlace válido.");
        ...
        if (dd.length > 10)
          res.redirect("?error=true&message=Se alcanzó el límite de tiempo de actividad.");

        if (dde)
          return res.redirect(
            "?error=true&message=Este enlace ya existe en el sistema."
          );
        ...
        res.redirect(
          "?success=true&message=Su enlace se ha agregado correctamente al sistema de tiempo de actividad."
        );
      }
    })
    ...
    app.post('/api/delete', async  (req, res) => {
      ...
      return res.json({
        ...
        alert: {
          title: "Success!",
          message: "¡El tiempo de actividad se ha eliminado correctamente!",
          type: "success"
        }
      })
    })
    ...
    app.post("/admin/maintence", checkMaintence, checkAuth, async (req, res) => {
      ...
      if(bakimdata) return res.json({ success: false, alert: { message: "El mantenimiento ya está habilitado!", type: "error", title: "Oops!" } })
      if(!req.body.reason) return res.json({ success: false, alert: { message: "No se proporcionó ninguna razón!", type: "error", title: "Oops!" } })
      new maintenceSchema({server: config.dashboard.discord_server_ID, reason: req.body.reason}).save();
      return res.json({ success: true, alert: { message: "¡Se ha habilitado el modo de mantenimiento!", type: "success", title: "¡Éxito!" }})
    })
